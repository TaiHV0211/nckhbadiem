        ...
        if ($row = mysqli_fetch_assoc($resultl)){
            ...
            if (!empty($row['username'])){
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    //Login or Logout
                    if ($row = mysqli_fetch_assoc($resultl)) {
                        // Cập nhật cột timeout nếu đã tồn tại record với cùng username và ngày hôm đó
                        ...
                        // Tạo record mới nếu chưa tồn tại record nào với cùng username và ngày hôm đó
                        $sql = "INSERT INTO users_logs (username, phonenumber, class, fingerprint_id, checkindate, timein, timeout) VALUES (? ,?, ?, ?, CURDATE(), CURTIME(), ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            $timeout = "";
                            mysqli_stmt_bind_param($result, "sssis", $Uname, $Number, $_class_,$fingerID, $timeout);
                            mysqli_stmt_execute($result);
            ...
            if (!mysqli_stmt_prepare($result, $sql)) {
                ...
                if ($row = mysqli_fetch_assoc($resultl)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $sql = "INSERT INTO users (username , phonenumber, gender, fingerprint_id, fingerprint_select) VALUES (?, ?, ?, ?, ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            mysqli_stmt_bind_param($result, "sssi", $Uname, $Number, $gender, $fingerID);
                            mysqli_stmt_execute($result);
